...
with tab2:
    ...
    st.markdown("The ESHAP Cross-Screen Attention Index (ESCAI) introduces a completely new analytical paradigm to capture this shift. We didn't build a local programmatic tool to place an individual ad spot next Tuesday. To try and use this index to execute a DSP trade is to confuse a compass with a shovel.")
    ...
    st.markdown("ESCAI enforces the absolute laws of human physics. Human time is a non-elastic, zero-sum commodity—a closed market budget. Every hour that a pair of eyes are focused on a phone screen, is an hour permanently eliminated from pool of available the TV time. Because, contrary to conventional wisdom, attention cannot actually be divided.")
