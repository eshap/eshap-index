...
# Helper function to safely read external copy
...
FR_RAW = {
    "France Télévisions": (510.0, 385.0), "YouTube": (485.0, 95.0), "TF1 Group": (440.0, 270.0),
    "Netflix": (390.0, 85.0), "TikTok": (335.0, 12.0), "Groupe M6": (265.0, 145.0),
    "Instagram": (215.0, 20.0), "Canal+ Group (Vivendi)": (195.0, 115.0), "Facebook": (165.0, 92.0),
...
def build_demographic_matrix(raw_data, shifts=None):
    ...
    for entity, (p13, p55) in raw_data.items():
        ...
        w13_44 = min(w13_54 * DECAY["13-44"], w13_54)
        w13_34 = min(w13_44 * DECAY["13-34"], w13_44)
        w13_24 = min(w13_34 * DECAY["13-24"], w13_34)
...
with tab1:
    ...
    chart_metrics = ["All P13+", "13-54 Workforce", "55+ Retirement"] if selected_demo == "Show All Cohorts Overlaid" else [selected_demo]
...
with tab2:
    ...
    with sub_method:
        ...
        if market_choice == "United States":
            ...
            st.write(load_text_asset("methodology_us.txt", "US copy file missing."))
        else:
            st.markdown("**Territorial Demographic Weight:** 65.1% of Population is ≤ 54 Years Old (34.9% is ≥ 55)")
            st.write(load_text_asset("methodology_fr.txt", "France copy file missing."))
        
    with sub_source:
        st.markdown("### 🔍 DATA SOURCES")
        if market_choice == "United States":
            st.write(load_text_asset("sources_us.txt", "US sources file missing."))
        else:
            st.write(load_text_asset("sources_fr.txt", "France sources file missing."))
