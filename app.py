...
with tab1:
    ...
    if market_choice == "Global Attention Index":
        # Fraction Conversion Engine: Converts 16.32 to 0.1632 so Altair plots the percentage natively
        render_df = chart_df.copy()
        for c in cols[1:]:
            render_df[c] = render_df[c] / 100.0
            
        # Field Property Lock: Natively parses space/plus string headers without column drops
        base_chart = alt.Chart(render_df).mark_bar(color="#FF0000").encode(
            ...
            tooltip=[
                ...
                alt.Tooltip(alt.Field(selected_demo, type='quantitative'), format=".2f%", title="Share")
            ]
