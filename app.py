...
with tab1:
    ...
    if market_choice == "Global Attention Index":
        # Fraction Scaler: Divides data by 100 so Altair plots the scale parameters accurately
        render_df = chart_df.copy()
        for c in cols[1:]:
            render_df[c] = render_df[c] / 100.0
            
        # Field Property Configuration: Safely binds column spaces and expands label spacing to 300px
        base_chart = alt.Chart(render_df).mark_bar(color="#FF0000").encode(
            x=alt.X(expr=f"datum['{selected_demo}']", type="quantitative", axis=alt.Axis(format=".1%"), title=None),
            ...
            tooltip=[
                ...
                alt.Tooltip(expr=f"datum['{selected_demo}']", type="quantitative", format=".2f%", title="Share")
            ]
    ...
    # Table Presentation Shield: Safely formats matrix rows as string percentages on the view layer
    if market_choice == "Global Attention Index":
