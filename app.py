...
with tab2:
    ...
    st.markdown("In the commercial Media industry, measurement has always been divided by screens [source: 1]. No major auditing bureau or currency panel has ever forced television meters, handset timestamps, and browser logs into a singular, logic-enforced, zero-sum attention budget [source: 1]. **This is collective malpractice [source: 1].** If you still treat social video and television as separate silos, to twist the age-old Wanamaker adage: *You are wasting half your money [source: 1].*")
    st.markdown("In the next three years, one trillion dollars will be spent on advertising in the US alone [source: 1]. More than three quarters of that will be spent on digital platforms [source: 1]. Another \$180 billion will be spent on television advertising [source: 1]. No one is measuring the reach, frequency, or effectiveness of this massive cashflow across these platforms [source: 1].")
    if os.path.exists("eshap_us_devices.png"): st.image("eshap_us_devices.png", caption="Video Consumption Share By Device Ecosystem (US Baseline) [source: 2]", use_container_width=True)
    else: st.info("💡 *[Placeholder for eshap_us_devices.png: In a recent survey of 3,000 US consumers, 59% watch video primarily on phones vs 28% on traditional TV glass [source: 2]]*")
    st.markdown("Publishers and advertisers look at their TV ratings, digital buyers and social media experts look at mobile handset logs, and everybody claims they are winning the war for reach [source: 1]. Meanwhile the measurement industry locks agencies into long-term contracts, then charges sellers through the nose to use their 'currency [source: 1].' Or, they will track attention across all the platforms, but only by campaign - not as a map nor trajectory of total cross-screen attention [source: 1]. This leaves our data on *real attention* siloed to the point of pointlessness [source: 1].")
    st.markdown("The industry is totally blind to how real people actually behave [source: 1]. Today's consumer is a singular, fluid entity moving seamlessly across a multi-screen day, allocating a single, finite commodity: **attention [source: 1].** The Media industry is lost specifically because they are charting their course based on the failed logic of vanity metrics, rather than following the actual data; the voice of the audience [source: 1].")
    if os.path.exists("eshap_devicesgen.png"): st.image("eshap_devicesgen.png", caption="Video Device Preference Layout Broken Down Across Generational Cohorts [source: 2]", use_container_width=True)
    else: st.info("💡 *[Placeholder for eshap_devicesgen.png: Media Generation Gap breakdown charting step-down linear curves from 18-24 up to Boomer demographics [source: 2]]*")
    st.markdown("Only Baby Boomers watch more video on televisions than on phones [source: 3]. Every other generation of Americans watches video on their phones two to three times more than their TVs [source: 3]. By treating human focus as a tangible physical property constrained by a fixed 24-hour clock, we built a model that removes corporate bias, complacency, and corruption from measurement [source: 3]. It exposes the actual depth of consumer focus, proving that while a traditional network may still technically 'reach' a young consumer, that consumer's finite daily time budget has completely migrated to a personal screen [source: 7].")
    st.markdown("Here's the craziest part of this (and the point of this exercise for us): The underlying raw inputs we used to construct this index — regulatory agency white papers, investor relations slide decks, quarterly financial reporting statements, public commission audits from accredited data providers, government agencies, public utilities, and published research — are all 100% publicly available, out in the open for anyone to access [source: 3]. Our methodology does not rely on secretly passed corporate logs or restricted database leaks, or paywalled data dashboards [source: 3]. Instead, we took fragmented fragments of data that sit in the open public domain and applied a proprietary mathematical synthesis that no one else has bothered to execute, until now [source: 3].")
    st.markdown("Why? Because our industry is kept in eat-what-you-kill silos [source: 4]. So we don't ask [source: 4]. Our data construct is sound [source: 4]. But the framework for this index is not built solely on quantitative analysis [source: 4]. Each year, we interview hundreds of experts, practitioners, vendors, buyers, managers, executives, interns, academics, data scientists, platforms, publishers, producers, and members of our community [source: 4]. On nearly every one of our working days [source: 4].")
    st.markdown("The numbers in this index are from companies we all know [source: 4]. So is the point of view [source: 4]. We ask the right questions — in this case, where is the attention of the whole consumer actually going — because everyone asked us [source: 4]. That fear of finding out is the systemic blindness now pushing our industry off a cliff [source: 4]. Thus, the ECSAI — the ESHAP Cross-Screen Attention Index [source: 4]. It's our new compass toward today's audience: The Whole Consumer [source: 4].")
