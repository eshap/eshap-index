...
# Unsplittable Production Asset Dynamic Loader Engine
def load_production_manifest(filename, fallback_msg="Content loading..."):
    if os.path.exists(filename):
        try:
            with open(filename, "r", encoding="utf-8") as f:
...
with tab2:
    ...
    # Dynamically injects Manifesto directly from your external file with zero code weight
    st.markdown(load_production_manifest("manifesto.txt", "Manifesto document loading..."))

with tab3:
    st.subheader("ECSAI Frequently Asked Questions (FAQs)")
    
    # Dynamically injects your complete FAQ Tree from your external file cleanly
    st.markdown(load_production_manifest("faqs.txt", "FAQs document loading..."))
