...
if merge_meta:
    ...
    if not meta_rows.empty:
        ...
        df_matrix = pd.concat([non_meta_df, pd.DataFrame([["META"] + summed_vals], columns=cols)], ignore_index=True).sort_values(by="All P13+", ascending=False).reset_index(drop=True)
...
if abs(total_shifted_hours) > 0.01:
    ...
    total_non_shifted_pool = float(df_static_base[non_shifted_mask]["All P13+"].sum())
    if total_non_shifted_pool > 0.0:
        for entity in df_static_base[non_shifted_mask]["Platform/Publisher"].unique():
            idx = df_matrix[df_matrix["Platform/Publisher"] == entity].index
            p13_orig_val = float(df_static_base.loc[idx, "All P13+"].values)
            ...
            df_matrix.loc[idx, "All P13+"] = p13_orig_val * ratio
            df_matrix.loc[idx, "13-54 Workforce"] = max(0.0, (p13_orig_val * ratio) - float(df_static_base.loc[idx, "55+ Layer"].values))
            for c in ["13-44 Youth", "13-34 Core", "13-24 Gen Z"]: df_matrix.loc[idx, c] = float(df_static_base.loc[idx, c].values) * ratio

df_matrix[cols[1:]] = df_matrix[cols[1:]].round(2 if market_choice == "Global Attention Index" else 1)
if abs(df_matrix["All P13+"].sum() - df_static_base["All P13+"].sum()) > 0.1: st.sidebar.warning("Simulated Shift Imbalance Detected")
...
with tab1:
    ...
    selected_demo = st.radio("Select Demographic Cohort to Isolate in Bar Chart:", options=cols[1:], horizontal=True)
    ...
    if market_choice == "Global Attention Index":
        ...
        for c in cols[1:]: render_df[c] = render_df[c] / 100.0
        base_chart = alt.Chart(render_df).mark_bar(color="#FF0000").encode(
            x=alt.X(alt.Field(selected_demo, type='quantitative'), axis=alt.Axis(format=".1%"), title=None),
            y=alt.Y("Platform/Publisher:N", sort="-x", title=None, axis=alt.Axis(labelLimit=300)),
            tooltip=[alt.Tooltip("Platform/Publisher:N", title="Publisher"), alt.Tooltip(alt.Field(selected_demo, type='quantitative'), format=".2f%", title="Share")]
        ).properties(height=380).configure_axis(labelFontWeight="bold")
    ...
    if market_choice == "Global Attention Index":
        ...
        for c in cols[1:]: display_df[c] = display_df[c].apply(lambda x: f"{x:.2f}%")
    ...
    if market_choice == "Global Attention Index":
        st.markdown("<p style='font-size: 0.82rem; font-style: italic; color: #444444; margin-top: 0.5rem; line-height: 1.4;'><strong>Cross Screen Attention Ledger: GLOBAL INDEX</strong><br>Values represent aggregated, normalized global attention share coefficients across tracking dimensions.</p>", unsafe_allow_html=True)
    elif market_choice == "Brazil":
        st.markdown("<p style='font-size: 0.82rem; font-style: italic; color: #444444; margin-top: 0.5rem; line-height: 1.4;'><strong>Cross Screen Attention Ledger: BRAZIL</strong><br>Platform totals represent unified corporate parent structures. Grupo Globo incorporates all Globoplay streaming telemetry.</p>", unsafe_allow_html=True)
    elif market_choice == "Mexico":
        st.markdown("<p style='font-size: 0.82rem; font-style: italic; color: #444444; margin-top: 0.5rem; line-height: 1.4;'><strong>Cross Screen Attention Ledger: MEXICO</strong><br>Platform totals represent unified corporate parent structures. TelevisaUnivision incorporates all ViX streaming telemetry.</p>", unsafe_allow_html=True)
    elif market_choice in ["France", "Germany", "United Kingdom", "Italy", "Spain"]:
        st.markdown(f"<p style='font-size: 0.82rem; font-style: italic; color: #444444; margin-top: 0.5rem; line-height: 1.4;'><strong>Cross Screen Attention Ledger: {market_choice.upper()}</strong><br>Platform totals represent unified holding corporate structures.</p>", unsafe_allow_html=True)
...
with tab2:
    ...
    st.markdown("We will update this index monthly, on a rolling six months basis.<br>Silultaneously, we will drop analysis of the latest data on **[Media War & Peace](https://substack.com)**.", unsafe_allow_html=True)
...
with tab3:
    ...
    st.markdown("#### Q: HOW DOES THE GENERATIONAL DECAY CALIBRATION RUN WITHOUT DATA LEAKS?")
    st.markdown("This is a crucial step to understanding how populations in different regions adapt to different media differently. To cut smoothly across the narrower demographic tiers (13-44, 13-34, 13-24) based only on public data, the model processes the numbers through localized fractional decay curves. Instead of assuming identical generational behavioral migration everywhere, the curves are adjusted using the baseline ratios found in the regulatory white papers (like Ofcom's Media Nations or Arcom's reports). The system applies a Nested Funnel Safety Guard: A strict structural logic barrier hardcoded into the model to ensure horizontal and vertical cohort integrity; this mathematically prevents narrower, younger generational slices of the population from ever showing a higher attention volume than the broader demographics that encapsulate them. This forces a mathematical step-down multiplier from left to right. This ensures that as the age bracket narrows, the legacy broadcaster values decay smoothly according to local policy protections, while the digital platform volumes scale up without causing a data inflation spill – more attention data than the population of any region could fill in any one day.")
    st.markdown("#### Q: HOW CAN THE INDEX CLAIM TO CAPTURE 'TOTAL ATTENTION' IF IT CAPS THE LIST AT 12 TO 15 CORPORATE PLAYERS PER COUNTRY?")
    st.markdown("We do not claim that this index tracks every niche website on the internet. It tracks the macroeconomic Market Attention Budget in these regions. ESHAP analysis focuses strictly on the tier-1 corporate holding structures that control more than 90% of commercial video and social media monetization. However, we did strive to examine more specialized attention providers like ARTE and L'ÉQUIPE. Smaller fragments across all regions are natively absorbed directly into the parent infrastructure or macro platform data where their video hours physically live. Beyond verified local exceptions, the long tail of micro-sites represents fragmented variables that do not, at the moment, alter the massive, shifting tectonic plates of the active attention budget in these major markets. We will continue to update this index with new entrants as often as possible. So, if you have specific recommendations, please send them to info@eshap.tv.")
    st.markdown("#### Q: WHY DO YOU USE 'MILLIONS OF HOURS' INSTEAD OF 'WEEKLY REACH' OR 'UNIQUE VISITORS' LIKE EVERY OTHER STANDARD CURRENCY?")
    st.markdown("Reach is an inflationary marketing metric designed to hide consumer abandonment. If a consumer opens a mobile application for five seconds to check a message, legacy metrics count them identically to a user who sits and streams video content for three hours. By converting all public data points into Absolute Attention Hours per Month, the index filters out the 'open app' illusion. It exposes the actual depth of consumer focus, proving that while a traditional network may still technically 'reach' a young consumer, that consumer's finite daily time budget has completely migrated to a personal screen.")
